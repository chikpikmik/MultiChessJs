...
if ((currentUserSide && currentUserSide.userReady) || !currentUserSide) readyButtonCont.remove()
...
if (currentUserSide) readyButtonCont.classList.remove('hidden')

if (isItCreator) displayedSides.classList.remove('hidden')
else {
...
function readyClick() {
    roomEl.sides.forEach((side) => {
        if (side.sideName === currentUserSide.sideName) side.userReady = true
    })
